...
class Cart extends \Magento\Framework\App\Action\Action
{
    ...
    {
        try {
            if ($cartHash = $this->getRequest()->getParam('qid', false)) {
                if ($this->restoreManager->restoreCart($cartHash)) {
                    $this->restoreManager->logEvent(
                        $this->getRequest()->getParam('msg', ''),
                        \Magetrend\AbandonedCart\Model\Queue::STATUS_CART_RESTORED,
                        true
                    );
