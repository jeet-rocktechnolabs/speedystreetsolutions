                    ...
                    if (count($oldBlocks) > 0){
                        ...
                        foreach ($oldBlocks as $old){
                            if ($overwrite){
                                ...
                                $num++; 
                                // $this->messageManager->addSuccess(__('Import overwrite Block Id (%1) Item(s) and Identifier "%2".', $old->getId(), $old->getIdentifier()));
                            } else {
                                $this->messageManager->addSuccess(__('Break overwrite Block Id (%1) Item(s) and Identifier "%2".', $old->getId(), $old->getIdentifier()));  
                            }
                        }
                        continue;
                    }
                    ...
                    if (count($oldPages) > 0) {
                        ...
                        foreach ($oldPages as $old){
                            if ($overwrite){ //If items can be overwrittenz
                                ...
                                $model->addData($item->asArray())->save();
                                $num++; 
                                // $this->messageManager->addSuccess(__('Import overwrite Page Id (%1) Item(s) and Identifier "%2".', $old->getId(), $old->getIdentifier()));
                            } else {
                                $this->messageManager->addSuccess(__('Break overwrite Page Id (%1) Item(s) and Identifier "%2".', $old->getId(), $old->getIdentifier()));
                            }
                        }
                        continue;
                    }
