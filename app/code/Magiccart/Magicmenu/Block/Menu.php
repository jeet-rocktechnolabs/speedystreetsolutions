        ...
        if($hasChild || $blocks['top'] || $blocks['left'] || $blocks['right'] || $blocks['bottom']) :
            $desktopTmp .= '<div class="level-top-mega">';  /* Wrap Mega */
                $desktopTmp .='<div class="content-mega">';  /*  Content Mega */
                    ...
                    $desktopTmp .= '<div class="content-mega-horizontal"><div class="p-cat-name">'.__($catTop->getName()).'</div>';
				...
				$imageUrl = $this->getFirstProductImageUrl($child);
if($imageUrl){
$img_str = '<span class="cat-img"><img src="'.$imageUrl.'" alt="'.__($child->getName()).'" /></span>';
} else {
$img_str = '';
}
$categoryDescription = $child->getDescription();

if ($categoryDescription != null && $categoryDescription != '') {
    $categoryDescription = strip_tags($categoryDescription);
    $words = explode(' ', $categoryDescription);
    $firstSevenWords = array_slice($words, 0, 7);
    $shortDescription = implode(' ', $firstSevenWords);
} else {
    $shortDescription =  "Browse ".__($child->getName())." product range.";
}
                                ...
                                $desktop_url =  '<a href="'. $child->getUrl().'">'.$img_str.'<span class="cat-name">'.__($child->getName()) . $this->getCatLabel($child) . '<br><span>'.$shortDescription.'</span></span></a>';
                                $url =  '<a href="'. $child->getUrl().'">'.__($child->getName()) . $this->getCatLabel($child) . '</a>';
                                ...
                                $desktopTmp .= '<li class="children ' . $class . '">' . $this->getImage($child) . $desktop_url . $childHtml . '</li>';
...
public function getFirstProductImageUrl($catObj)
    {
        //$category = $this->_categoryFactory->create()->load($categoryId);
        $collection = $this->_productCollectionFactory->create();
        $collection->addCategoryFilter($catObj);
        $collection->addAttributeToSelect('image');
        $collection->setPageSize(1); // Limit the collection to 1 product
        ...
        return false; // Return false or a placeholder image if no products are found
    }
